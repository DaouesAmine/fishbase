...
/**
 * Base class that represents a row from the 'data2050' table.
 *
 *
 *
 * @package    propel.generator.fbapp.om
 */
abstract class BaseData2050 extends BaseObject implements Persistent
{
    /**
     * Peer class name
     */
    const PEER = 'Data2050Peer';
     ...
     * @return Data2050 The current object (for fluent API support)
     */
    public function setSpc100($v)
    {
        ...
        if ($this->spc100 !== $v) {
            $this->spc100 = $v;
            $this->modifiedColumns[] = Data2050Peer::SPC100;
        }
    ...
    public function setCid($v)
    {
        ...
        if ($this->cid !== $v) {
            $this->cid = $v;
            $this->modifiedColumns[] = Data2050Peer::CID;
        }
    ...
    public function setProxcell($v)
    {
        ...
        if ($this->proxcell !== $v) {
            $this->proxcell = $v;
            $this->modifiedColumns[] = Data2050Peer::PROXCELL;
        }
    ...
    public function setPid($v)
    {
        ...
        if ($this->pid !== $v) {
            $this->pid = $v;
            $this->modifiedColumns[] = Data2050Peer::PID;
        }
    ...
    public function setSpc1001($v)
    {
        ...
        if ($this->spc100_1 !== $v) {
            $this->spc100_1 = $v;
            $this->modifiedColumns[] = Data2050Peer::SPC100_1;
        }
    ...
    public function setPkid($v)
    {
        ...
        if ($this->pkid !== $v) {
            $this->pkid = $v;
            $this->modifiedColumns[] = Data2050Peer::PKID;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 6; // 6 = Data2050Peer::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating Data2050 object", $e);
        }
    }
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        $stmt = Data2050Peer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = Data2050Query::create()
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                Data2050Peer::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(Data2050Peer::SPC100)) {
            $modifiedColumns[':p' . $index++]  = '`spc100`';
        }
        if ($this->isColumnModified(Data2050Peer::CID)) {
            $modifiedColumns[':p' . $index++]  = '`cid`';
        }
        if ($this->isColumnModified(Data2050Peer::PROXCELL)) {
            $modifiedColumns[':p' . $index++]  = '`proxcell`';
        }
        if ($this->isColumnModified(Data2050Peer::PID)) {
            $modifiedColumns[':p' . $index++]  = '`pid`';
        }
        if ($this->isColumnModified(Data2050Peer::SPC100_1)) {
            $modifiedColumns[':p' . $index++]  = '`spc100_1`';
        }
        if ($this->isColumnModified(Data2050Peer::PKID)) {
            $modifiedColumns[':p' . $index++]  = '`PKID`';
        }

        $sql = sprintf(
            'INSERT INTO `data2050` (%s) VALUES (%s)',
    ...
    protected function doValidate($columns = null)
    {
        if (!$this->alreadyInValidation) {
            ...
            if (($retval = Data2050Peer::doValidate($this, $columns)) !== true) {
    ...
    {
        $pos = Data2050Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['Data2050'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Data2050'][$this->getPrimaryKey()] = true;
        $keys = Data2050Peer::getFieldNames($keyType);
        $result = array(
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(Data2050Peer::SPC100)) $criteria->add(Data2050Peer::SPC100, $this->spc100);
        if ($this->isColumnModified(Data2050Peer::CID)) $criteria->add(Data2050Peer::CID, $this->cid);
        if ($this->isColumnModified(Data2050Peer::PROXCELL)) $criteria->add(Data2050Peer::PROXCELL, $this->proxcell);
        if ($this->isColumnModified(Data2050Peer::PID)) $criteria->add(Data2050Peer::PID, $this->pid);
        if ($this->isColumnModified(Data2050Peer::SPC100_1)) $criteria->add(Data2050Peer::SPC100_1, $this->spc100_1);
        if ($this->isColumnModified(Data2050Peer::PKID)) $criteria->add(Data2050Peer::PKID, $this->pkid);
    ...
    {
        $criteria = new Criteria(Data2050Peer::DATABASE_NAME);
        $criteria->add(Data2050Peer::PKID, $this->pkid);
     ...
     * objects.
     *
     * @param object $copyObj An object of Data2050 (or compatible) type.
     ...
     *
     * @param boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return Data2050 Clone of current object.
     * @throws PropelException
     */
    ...
    {
        return (string) $this->exportTo(Data2050Peer::DEFAULT_STRING_FORMAT);
    }
